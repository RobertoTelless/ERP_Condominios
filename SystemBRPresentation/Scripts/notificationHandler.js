...
function CallNotification() {
    ...
    $.ajax({
        url: '../Agenda/GetAgendaUsuario'
        , type: 'POST'
        , success: function (r) {
            if (r.msg != undefined) {
                notifier.show('Agendas', r.msg + '<span style="margin-left: 10px;" onclick="ViewNotificationAgenda()" class="btn btn-warning btn-sm">Ver</span>', 'success', '', 1000 * 15);
            }
        }
    });
}
...
function ViewNotificationAgenda() {
    window.open('../Agenda/MontarTelaAgenda', '_self');
}